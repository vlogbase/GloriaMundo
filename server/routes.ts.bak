...
Format your responses using markdown for better readability and organization.`;
      */
      ...
      try {
        ...
        // Process the chunks and forward them
        try {
          // Start async title generation (don't await here to avoid blocking stream)
          ...
          while (true) {
            ...
            // Forward each line from the chunk
            const lines = chunk.split('\n');
            for (const line of lines) {
              if (line.startsWith('data: ')) {
                // Forward the data event to the client
                res.write(line + '\n\n');
                
                // Parse the event data if needed for tracking
                try {
        ...
        // This code is no longer needed here as we handle title generation above
        /* Unreachable code - keeping as reference
